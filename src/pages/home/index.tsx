    ...
    return faqs.map((item: any) => {
      return <Col key={item.id}>
        <div className='d-flex align-items-center'>
          ...
          <span onClick={() => openTab(item.id)} className='FAQs-collapseHeader'>{item.Question}</span>
        </div>
        <Collapse in={open === item.id}>
          <div id="FAQs-collapse-text" className='px-2' style={{ margin: '0rem 2.5rem 1.5rem', borderLeft: '3px solid orange', fontSize: '1rem', color: 'gray' }}>
            <p>
              {item.Answer}
            </p>
          </div>
  ...
  return (
    <div style={{ width: '90%' }} className='FAQs pt-5 mx-auto' id='FAQs'>
      <Row className='mt-5'>
        ...
        <Col xs={12} lg={7}>
          <Row className='flex-column question-content'>
            {renderQAs()}
          </Row>
        </Col>
      </Row>
